...
def lstmnn(tick,epoch_no):
    ...
    #edir_path_1 = os.path.dirname(os.path.realpath(__file__))
    #epath_1 = edir_path_1 + '\\edata\\emer_data.csv'
    #data = pd.read_csv(epath_1)

    indx = get_history(symbol='NIFTY 100', start = date(2017,2,2), end = date.today(), index = True)

    #edir_path_2 = os.path.dirname(os.path.realpath(__file__))
    #epath_2 = edir_path_2 + '\\edata\\emer_nif.csv'
    #indx = pd.read_csv(epath_2)
    ...
    return yp_traina,nifty,orig,corr, main_op_a, main_corr,history.history['loss'], y_traina, five_day_md 
